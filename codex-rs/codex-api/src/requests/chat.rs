//! Request body + header assembly for the Chat Completions wire API.
//!
//! [fraktal] Restored after upstream's Responses-only consolidation (#7782) so
//! Fraktal can target OpenAI-compatible Chat Completions providers directly.
//! Ported from the pre-removal `codex-api` and adapted to the current
//! `EndpointSession`/header plumbing.
            ...
            match item {
                ...
                ResponseItem::FunctionCallOutput { call_id, output, .. } => {
                    messages.push(json!({
                        ...
                        "content": function_output_content_value(output),
                    }));
                }
                ...
                // Reasoning, web search, ghost snapshots, compaction markers and
                // any future items have no Chat Completions representation.
                _ => continue,
            }
        }
...
/// Renders a function/custom tool output payload into the Chat Completions
/// `content` field — either a plain string or an array of structured content
/// parts. Encrypted content items have no chat representation and are dropped.
fn function_output_content_value(output: &FunctionCallOutputPayload) -> Value {
    match &output.body {
